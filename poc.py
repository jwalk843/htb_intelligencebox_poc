...
while True:
    ...
    if r.status_code == 200:
        ...
        # after 15 pdfs are downloaded, kill while loop
        if len(os.listdir(".")) >= 16:
            print("\nI have enough PDF's downloaded!\nNow to Find some users :)\n")
            users = []
            for x in os.listdir():
                if x.endswith(".pdf"):
            ...
            print(f"[X] {pdf} File already Downloaded")
            pass
    ...
    """
            for line in read_file:
                ...
                if b"Creator" in line:
                    # print out the lines
                    print(line[0:50])
                    users.append(line[0:50].strip(b'\n'))
